...
class LLPLModel:
    ...
    def _check_data_columns(self):
        ...
        print(f"\n检测到的数据列: {', '.join(columns)}")
        
        # 检查是否有日期列
        date_cols = [col for col in columns if any(keyword in col.lower() for keyword in ['date', 'time', '日期', '时间'])]
        if date_cols:
            print(f"检测到日期列: {', '.join(date_cols)}")
        else:
            print("警告: 未检测到明确的日期列")
            
        # 检查是否有价格列
        price_cols = [col for col in columns if any(keyword in col.lower() for keyword in ['price', '价值', '价格', '收盘', 'close'])]
        if price_cols:
            print(f"检测到价格列: {', '.join(price_cols)}")
        else:
            print("警告: 未检测到明确的价格列")
            
        # 检查是否有成交量列
        volume_cols = [col for col in columns if any(keyword in col.lower() for keyword in ['volume', '交易量', '成交', 'vol'])]
        if volume_cols:
            print(f"检测到成交量列: {', '.join(volume_cols)}")
        else:
...
def main():
    ...
    if prices is not None:
        ...
        # 保存结果
        save_choice = input("\n是否保存结果到Excel文件? (y/n): ").strip().lower()
        if save_choice == 'y':
